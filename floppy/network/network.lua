...
local pingid = 0
function icmp.ping(addr, payload)
    pingid = pingid + 1
...
function tcp.unlisten(port)
    internal.udp.checkPortRange(port)
    internal.tcp.ports[port] = nil
end

function tcp.open(addr, port)
    internal.udp.checkPortRange(port)
...
function tcp.send(channel, data)
...
---@return table
function info.getInfo() end

---@param interface string
---@return table
function info.getInterfaceInfo(interface) end

---@return table
function info.getRoutes() end

---@param interface string
---@return table
function info.getArpTable(interface) end
